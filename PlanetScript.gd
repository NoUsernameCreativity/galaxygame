...
func GenerateStats():
	...
	var keysList = stats.keys()
	# shuffle the list and then use to randomly assign stats
	keysList.shuffle()
	for i in range(len(keysList)):
		if i < largerStatsNum:
			stats[keysList[i]] = 50+50*(randf()**2)
		else:
			stats[keysList[i]]=50*(randf()**2)
...
func GenerateMesh():
	...
	# set shader uniforms
	var mat = planetMesh.mesh.surface_get_material(0)
	...
	# set colours
#	mat.set_shader_parameter("baseColor", coloursList[rng.randi_range(0, len(coloursList)-1)])
#	mat.set_shader_parameter("nearWaterColor", coloursList[rng.randi_range(0, len(coloursList)-1)])
#	mat.set_shader_parameter("slopeColor", coloursList[rng.randi_range(0, len(coloursList)-1)])
	mat.set_shader_parameter("sandHeight", radius+0.2)
...
func DisplaceTerrain(vertices):
	...
	$gravityVolume/CollisionShape3D.shape.radius = radius * 2 # randomise
	$gravityVolume.gravity_point_unit_distance = radius / 2 # surface gravity
	$gravityVolume.gravity = randf_range(8, 10)
